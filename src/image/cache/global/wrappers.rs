...
impl StableImageBytes {
    const PRE_DECODED_KIND: u8 = 0;
    const COMPRESSED_SVG_KIND: u8 = 1;
...
impl TryFrom<StableImageBytes> for StableImage {
    ...
    fn try_from(bytes: StableImageBytes) -> Result<Self, Self::Error> {
        ...
        let kind = bytes.pop().context("Missing footer")?;
        if kind == StableImageBytes::PRE_DECODED_KIND {
            anyhow::ensure!(
                bytes.len() >= StableImageBytes::FOOTER_SIZE,
                "Invalid pre-decoded footer"
            );
            let (dim_x, dim_y, scale) = {
                let mut footer = bytes.drain(bytes.len() - StableImageBytes::FOOTER_SIZE..);
                let scale = match footer.next().expect("Length pre-checked") {
                    0 => false,
                    1 => true,
                    _ => anyhow::bail!("Invalid pre-decoded footer"),
                };
        ...
        } else if kind == StableImageBytes::COMPRESSED_SVG_KIND {
            Ok(Self::CompressedSvg(bytes))
        } else {
            Err(anyhow::anyhow!("Invalid footer kind"))
        }
    }
}
    ...
    fn from(data: StableImage) -> Self {
        match data {
            StableImage::PreDecoded(ImageData {
                ...
                bytes.reserve_exact(Self::FOOTER_SIZE + 1);
